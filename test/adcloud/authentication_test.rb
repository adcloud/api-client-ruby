...
describe Adcloud::Authentication do
  ...
  describe "connection" do
    ...
    subject { Adcloud::Authentication.new(:client_id => "0987654321", :client_secret => "1234567890").connection }

    describe "not logged connection" do

      it "should open a connection to https://api.adcloud.net:80/" do
        subject.url_prefix.to_s.must_equal "https://api.adcloud.net:80/"
      end

      it "shouldn't have a handler" do
        subject.builder.handlers.wont_include(Faraday::Response::Logger)
      end
    ...
    describe "logged connection" do
      ...
      it "should have a logger handler" do
        ...
        subject.builder.handlers.must_include(Faraday::Response::Logger)
      end
