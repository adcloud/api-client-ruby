...
describe Adcloud::Campaign do
  ...
  describe "all" do
    ...
    it "should return an empty array if response is empty" do
      subject.connection.expects(:get).with('campaigns', :filter => {}).returns(empty_response_data)
      subject.all.must_be_instance_of(Array)
    end

    it 'should return an array if response contains objects' do
      subject.connection.expects(:get).with('campaigns', :filter => {}).returns(response_data)
      subject.all.must_be_instance_of(Array)
    end
    ...
    it 'should pass the filter param' do
      subject.connection.expects(:get).with('campaigns', :filter => {:oliver => "ist chef"}).returns(response_data)
